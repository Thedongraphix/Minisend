          ...
          <AnimatePresence>
            {showDropdown && (
              <motion.div
                ...
                transition={{ duration: 0.2, ease: "easeOut" }}
                className="absolute top-full left-0 mt-2 w-36 bg-[#1c1c1e] border border-[#3a3a3c] rounded-xl shadow-2xl overflow-hidden"
              >
                {AVAILABLE_COUNTRIES.map((country) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      <div className={`text-xs font-medium ${selectedCountry === country.code ? 'text-white' : 'text-[#8e8e93]'}`}>
                        {country.name}
                      </div>
                      <div className="text-[10px] text-[#8e8e93]">{country.countryCode}</div>
                    </div>
                    {selectedCountry === country.code && (
                      <svg className="w-3.5 h-3.5 text-[#0066FF]" fill="currentColor" viewBox="0 0 20 20">
